...
| `.time-band` > `.chip`(+`.accent`) + `.dg-arrow` | 진행 시간 띠 한 줄(화살표 28px). S41–S43 공통, 현재 구간 칩만 accent |
| `.time-blocks.row .time-block` | 4칸 한 줄 시간 블록 좌우 여백 16px (S37) |
...
- 글자 크기 종류(오케스트레이터 결정, 2026-09-16): 장면당 4종 기준에 v0.4 설명·관찰할 것 층(32px)을 추가 층으로 더해 **5종까지 허용**한다. 실습 안내판(단계 목록 + 관찰할 것)과 callout이 있는 장면이 해당한다. 6종 이상이면 줄인다.
