...
## 7-1. v0.4 진행 상태 (2026-09-16)

v0.3 52장을 `docs/slide-plan-v0.4-visual-narrative.md`(12-1절 사용자 답변 9건)에 따라 57장으로 개정했다. 브랜치 `feat/sk-hynix-deck-v0.4`.

| 항목 | 결과 |
|---|---|
| 장면 | 57장 (본문 47 + 부록 10). G00(G01 다음)과 A07–A10 추가, `#root` 285초 |
| 텍스트 | 막 태그 7곳, 장면마다 한 줄 요지·설명 문단(실습 안내판은 "관찰할 것") |
| 시각 요소 | 모든 장면 1개 이상. 인라인 `.dg` 도식, 원 논문 그림(S05·A01), 참고 영상 썸네일(G00), 승인 카드 그림(A10), 플레이스홀더 10곳 |
| 검토 | 차수 묶음 5개(B1 / B2+B3 / B4+B5 / B6+B7 / 부록)마다 강사 사전·사후 검토 반영 |
| 검증 | validate 통과, lint 오류 0, `--check` 최신, Playwright 넘침 57장 0건 |

남은 일은 v0.3과 같다(사용자 overview 검토 → 확인 목록 답변·자리표시·캡처 교체 → 최종 확인 후 export). 확인 목록은 `_workspace/ppt_qa_report.md` "v0.4" 절에 있다.
