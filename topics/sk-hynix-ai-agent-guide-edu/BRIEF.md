...
### 현재 구성 (v0.8 · 2026-09-18 · `ROADMAP3.md`)

실제 실습자료(Part 4개 · 실습 9개)와 번호 · 순서 · 말을 맞춘 구성이다. 66장 = 본문 49 + 구간 표지 7 + 부록 10. 진행 축은 **LLM → RAG → Tool → Agent → Coding Agent → 적용** 이고(v0.7에서 비유 축을 전문용어로 교체), 주제가 바뀌는 지점마다 짙은 주황 구간 표지(D01~D07)가 진행 띠와 함께 나온다. 표지 부제는 `실습 NN · <그 구간에서 배우는 것>` 형식으로, 용어만 보고는 무엇을 배우는지 모르던 문제를 받아 준다. 화면의 시각은 G01(시작 · 점심 · 종료)과 G03(점심 안내)에만 있고, 분 배분은 `docs/instructor-runsheet.md`(강사 진행표)에만 있다.

| 구간 | 실습자료 | 표지 | 장면 |
|---|---|---|---|
| 도입 | — | — | S01 · G01 · S02 · G00 |
| 준비 | 실습 01 | D01 | E01 · LAB01 |
| LLM | 실습 02 · 03 | D02 | S04 · N01 · LAB02 · S07 · L01 · LAB03 · T01 |
| RAG | 실습 04 | D03 | S10 · V01 · V02 · R01 · LAB04 · S14 · T02 |
| Tool | 실습 04b | D04 | S15 · S16 · LAB04b · N02 · T03 |
| Agent | 실습 05 | D05 | N03 · S29 · LAB05 · S17 · S21 · G03(점심) |
| Coding Agent | 실습 06 · 07 | D06 | S22 · S23 · LAB06 · LAB07 · C01 · C02 · S25 · S28 · C03 · S30 · C04 |
...
v0.6 재구성(2026-09-18, `ROADMAP2.md`): 실제 실습자료 요약본(Part 4 · 실습 9개)에 맞춰 55장 → **66장**(본문 49 + 구간 표지 7 + 부록 10). 실습 번호를 실습자료와 같게(01~08 · 04b), 코드 위치는 "실습자료 · 실습 NN". RAG는 검색 → 근거 → 답변 흐름, Tool은 계산 도구 3종과 데이터 직접 조회, Agent 구간 신설, 긍정봇은 temperature · 페르소나 · 답의 틀 관찰, 오전과 오후가 같은 게이트웨이 · 같은 인증 키. 비유 축과 구간 표지 도입, 전역 UI 패치(상자 테두리 대비 3:1 · 가운데 정렬 · 썸네일 모서리). 교육설계 서브에이전트 `llm-edu-designer`가 검수 · 스토리라인 · 콘티를 맡고 `llm-professional` · `lecture_expert`가 교차 검수. QA 게이트 통과.

v0.7 전문용어 교체(2026-09-18, `ROADMAP3.md`): 장수는 66장 그대로 두고 **비유 표현을 전부 전문용어로 바꾼 판**. "제목부터 비유라서 무엇을 배우는지 알기 힘들다"는 사용자 지적에서 출발했다. 진행 축 `두뇌·책·손발·조종·내 PC의 에이전트` → **`LLM · RAG · Tool · Agent · Coding Agent`**, 열쇠·문 → 인증 키·게이트웨이, 정거장 → 단계, 하루 흐름 → 강의 순서. 치환 표 268항목(`_workspace/roadmap3/tables/`)을 `apply_table.py`로, 아이콘 대체 글자 36곳 · 띠 라벨 10곳 · aria-label 4곳 · CSS 주석 9곳을 `bulk_replace.py`로 처리했다(아이콘 그림 파일 252개는 그대로). 문장을 늘리지 않고 덜 중요한 문장을 지우는 원칙으로 화면 글자 −371자 · 노트 −437자, 핵심 흐름 9종(완료 조건 30 · 막히면 17 · 관찰할 것 12 · 자리표시 13 · 출처 49 · 개념 칩 21 · 코드 위치 18 · 환경 파일 안전 15 · 실습 번호 72) 개수는 v0.6과 완전 일치. 인증 수단은 정의 장면 E01만 긴 이름, 나머지는 짧은 이름. 구간 표지 7장에 `실습 NN · <배우는 것>` 부제를 붙여 비유가 하던 안내 역할을 대신하게 했다. 검수 세 갈래(`llm-professional` 용어 · `llm-edu-designer` 교육설계 · `slide_ui_designer` ×2 Playwright 시각)를 거쳤고 배치 보정은 `slide_ui/06_v07_fixes.css` · `07_v07_visual.css`. 되돌릴 지점은 태그 `v0.6-metaphor`, v0.6 원본 백업 `_workspace/roadmap3/backup_v0.6/`.

v0.8 한 사람 진행(2026-09-18, `ROADMAP3.md` 15절): **짝 활동과 손들기를 없애 한 사람이 혼자 진행하는 형태로** 바꿨다. 사용자 지시는 "이 강의는 1명이서 모든 걸 진행하는 걸 가정한다"였다. 대상 57항목(장면 30) — 짝·옆사람 20 · 손들기·손가락 20 · 지목·응답 요구 11 · 마무리 장면 6. **질문 자체는 남기고 답하는 방식만** 없앴고, 손가락으로 답하게 한 질문을 뒤에서 회수하는 짝(LAB04→S14 · LAB04b→N02 · S14→T02 · S44→G02)은 회수 문장을 함께 고쳐 살렸다. 막힘 대처의 `짝 화면으로`는 통째로 삭제하고, `옆 사람에게 말해 주세요`는 혼자 하는 활동으로 바꿨다(업무 적용형 S15·S17·S39는 산출물이 남도록 "적기"로). 마지막 장면 S44는 `사례 공유 · Q&A · 마무리` → `정리 · 마무리`로 바꿔 발표 전제를 없앴다. 화면 −125자 · 노트 −635자, 잔존 0건, `막히면` 칸 12개와 핵심 흐름은 그대로. 손대지 않은 오탐: `짝지어`(대응시키다) · 코드 안 프롬프트 문자열 · Claude Code 요청문 예시 · 강사가 일방적으로 보여 주는 진행. v0.7 원본 백업 `_workspace/roadmap3/backup_v0.7/`. 사용자 overview 검토 · export 대기.
