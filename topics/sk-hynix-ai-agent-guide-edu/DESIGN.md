...
| `--accent` | `#0075de` | 유일한 구조 액센트. 강조 밑줄, 단계 번호, 현재 위치 표시 |
| `--accent-soft` | `rgba(0,117,222,0.08)` | 액센트 배경 틴트 |
| `--night` | `#213183` | 반전 화면 배경. S01 표지와 G02 마무리에만 |
| `--ok` / `--warn` | `#1aae39` / `#dd5b00` | 비교 화면의 상태 점 전용. 글자·면 채우기 금지 |
| `--line-strong` | `#c8c4bf` | 점선 플레이스홀더·개념도 영역 테두리 (중립 파생값) |
| `--on-accent` / `--on-night` | `#ffffff` | 파랑 면·남색 면 위 글자 |
...
- 그림자 없음. 면 구분은 헤어라인과 흰 카드로만 한다.
- 여백은 8px 배수. 장면 안쪽 여백 상하 96px, 좌우 120px.

### 금지

- 스티커 팔레트(보라·분홍·주황·청록·초록·하늘)를 장식·면 채우기에 쓰지 않는다.
- 파랑 외의 두 번째 구조 액센트를 만들지 않는다.
- 남색 반전 밴드를 S01·G02 외 장면에 쓰지 않는다.
