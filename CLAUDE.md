...
- **화면이 넘칠 때 빼는 순서**: 설명문 → 실습자료 하위 번호 줄 → 출처 줄 → 소단계 아이콘 → 열쇠 지도 칩 줄 → 개념 칩 줄. 개념 칩(소단계 번호 + 직전 개념)은 "코드만 치고 개념은 모르는 상태"를 막는 장치이므로 마지막까지 남긴다. 공간은 글을 빼기 전에 장면 전용 압축 CSS(글자 24px 이상)로 먼저 만든다.
- **열쇠가 든 환경 파일은 Claude Code에게 읽히지 않는다.** 오전 실습 폴더를 여는 장면의 요청문 예시에는 "환경 파일은 열지 마세요"를 넣는다.
