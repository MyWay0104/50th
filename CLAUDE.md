...
- **실습 자료는 사내 전용**이다. 실습의 기준은 실습자료 요약본(`docs/ai-agent-guide-실습자료-강의흐름요약.md`)과 topic의 `ROADMAP2.md` 0절 확정 사실이다. 현업 가이드(`skh_llm_guide`)는 다른 자료이므로 실습 코드로 인용하지 않는다. **덱의 실습 번호는 실습자료와 같게**(실습 01~08 · 04b) 쓰고, 코드 위치는 **"코드 위치: 실습자료 · 실습 NN"** 한 가지 형식으로만 적는다. 폴더·파일·함수의 실제 이름은 화면과 노트에 쓰지 않는다.
- **시간 정보는 첫 시간표(G01)와 점심 안내(G03)에만** 둔다. 실습 시간은 실제와 달라 수강생에게 압박이 된다. 나머지는 순서 표현(먼저·이어서·다음 구간)으로 쓴다. 분 배분은 `docs/instructor-runsheet.md`(강사 진행표)에만 둔다.
- **화면이 넘칠 때 빼는 순서**: 설명문 → 실습자료 하위 번호 줄 → 출처 줄 → 소단계 아이콘 → 인증 키 지도 칩 줄 → 개념 칩 줄. 개념 칩(소단계 번호 + 직전 개념)은 "코드만 치고 개념은 모르는 상태"를 막는 장치이므로 마지막까지 남긴다. 공간은 글을 빼기 전에 장면 전용 압축 CSS(글자 24px 이상)로 먼저 만든다.
- **인증 키가 든 환경 파일은 Claude Code에게 읽히지 않는다.** 오전 실습 폴더를 여는 장면의 요청문 예시에는 "환경 파일은 열지 마세요"를 넣는다.
- **비유로 개념을 대신하지 않는다** (v0.7, 2026-09-18 사용자 결정). 제목·진행 띠·본문은 전문용어로 쓰고(`LLM` · `RAG` · `Tool` · `Agent` · `Coding Agent` · `적용`), 바로 아래 한 줄이 그 용어를 평이한 우리말로 받아 준다. 비유를 설명 보조로 쓸 때는 그것이 비유임을 화면에 밝히지 말고 정확한 정의를 먼저 준다. 인증 수단은 정의 장면(E01)에서만 긴 이름(`채팅용 API 키`)을 쓰고 나머지는 짧은 이름(`채팅용 키` · `임베딩용 키` · `데이터 조회용 토큰`)으로 통일한다.
- **강의는 한 사람이 혼자 진행한다** (v0.8, 2026-09-18 사용자 결정). 다른 수강생이나 강사의 반응이 있어야 성립하는 문장을 쓰지 않는다 — 짝을 지어 확인하기, 손을 들어 답하기, 여러 사람 중 한 명을 지목해 묻기가 모두 금지다. **질문 자체는 쓰되 답하는 방식을 요구하지 않는다**(물음표로 던지고 강사가 스스로 답하며 넘어간다). 수강생 활동은 말하기가 아니라 **적기·고르기·짚기**로 쓴다. 다만 `짝지어`(대응시키다) · 코드 안의 프롬프트 문자열 · Claude Code에게 주는 요청문 예시 · 강사가 일방적으로 보여 주는 진행(`제 화면으로 보겠습니다`)은 해당하지 않는다.
- **배치 기준** (2026-09-18 사용자 지시): 텍스트를 한쪽으로 몰지 않고 균형 잡힌 중앙 정렬로 둔다. 컴포넌트끼리 다닥다닥 붙이지 않는다. 글을 지워 생긴 여유는 간격을 넓히는 데 쓰고 글자를 키우지 않는다. 최종 UI 검토에는 **Playwright로 장면별 시각 검토를 반드시 포함**한다(`hyperframes check --samples 66`은 기계 지표만 보므로 세로 무게 중심·고아 줄바꿈·빈 칸은 눈으로 봐야 잡힌다).
...
- 강조색은 주황 `--accent: #dd5b00`, 반전 배경은 `--night: #793400` 이다(`DESIGN.md` 참고). 반전 배경은 표지 S01 · 마무리 G02 · 구간 표지 D01–D07에만 쓴다.
- **상자류 테두리는 배경 대비 3:1 이상**(`--box-line`), 상자 안 내용은 상하좌우 가운데 정렬, 사진·썸네일은 네 모서리 모두 둥글게 한다(2026-09-18 사용자 지시).
