...
| `sk-hynix-ai-agent-guide-edu` (v0.4) | `docs/slide-plan-v0.4-visual-narrative.md` | 57장(G00·A07–A10 추가, `#root` 285초), G00은 G01 다음, 모든 장면에 시각 요소 1개 이상, 텍스트 6층(한 줄 요지·설명 문단 추가), 기-승-전-결 막 태그. 결정 원본은 12-1절. **상태(final-touch, 2026-09-17)**: 57장, 사용자 수정 12건 반영(주황 테마 `#dd5b00`·구간 표시·쪽번호·시간 정보 삭제·존댓말·공식 출처만·로고), main 병합, 사용자 overview 검토 대기. 기록 `_workspace/final_touch.md`. 사용자 확인 목록은 `_workspace/ppt_qa_report.md`의 "v0.4" 절 |
